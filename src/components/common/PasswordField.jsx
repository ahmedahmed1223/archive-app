 ...
 *
 * - Surface uses `.va-surface-deep` + `--va-2-*` tokens, so it follows the
 *   active light/dark theme (no hardcoded dark colors that bleed in light).
 * - The reveal button sits on the inline-start (visual left in the RTL UI)
 *   with matching padding so it never overlaps the typed value.
    ...
    children: [
      jsx("input", {
        ...
        className:
          ...
          `text-[color:var(--va-2-text-strong)] placeholder:text-[color:var(--va-2-text-faint)] ` +
          `focus:border-[color:var(--va-2-accent)] ${inputClassName}`
      }),
      jsx("button", {
        type: "button",
        ...
        className:
          "absolute inset-inline-start-2 top-1/2 inline-flex h-7 w-7 -translate-y-1/2 items-center justify-center " +
          "rounded-lg text-[color:var(--va-2-text-soft)] transition-colors hover:text-[color:var(--va-2-text-strong)] " +
          "hover:bg-[color:var(--va-2-line)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--va-2-accent)]",
